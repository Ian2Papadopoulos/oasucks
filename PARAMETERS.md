...
breaks if you change it. Values here are the **v92 defaults** — if you edit the
...
| `ACT_TTL.getStopArrivals` | 12 s | **90 s** | Above the refresh interval on purpose, so two people at the same stop cost one call rather than two. Raised from 50 in v92, and only safe because of v79: the age of a cached answer is subtracted from the minutes before anyone sees them, so a 90-second cache shows the same countdown a 50-second one did. |
...
edge cache actually being hit, and until v92 that was an assumption rather than a
